...
static int8_t meter_help_ua_dc_get_power(ao_meter_t *const me, uint8_t range);
...
void meter_ua_dc_init(ao_meter_t *const me) {
    ...
    me->es232_power_rel = meter_help_ua_dc_get_power(me, me->es232_range_min);
...
QState meter_ua_dc_adc(ao_meter_t *const me) {
    ...
    if (!me->es232_hold_flag) {
        me->es232_value_now = meter_help_ua_dc_cal(me, sadc_data);
        me->es232_power_now = meter_help_ua_dc_get_power(me, me->es232_write_buffer.range_msb);
    }
...
QState meter_ua_dc_key(ao_meter_t *const me) {
    ...
    switch (Q_PAR(me)) {
        case button_select_id << 4 | SINGLE_CLICK:
            QACTIVE_POST(me, AO_METER_MODE_SIG, meter_mode_ua_ac);
