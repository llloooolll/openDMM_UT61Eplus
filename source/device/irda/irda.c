...
static void irda_gpio_deinit(void) {
    gpio_set_mux(IRDA_UART_TXD_PORT, IRDA_UART_TXD_PIN, gpio_mux_0);
    gpio_set_mux(IRDA_UART_RXD_PORT, IRDA_UART_RXD_PIN, gpio_mux_0);

    gpio_clear_pin(IRDA_HALL_PWR_PORT, IRDA_HALL_PWR_PIN);  // 霍尔电源
    gpio_clear_pin(IRDA_POWER_EN_PORT, IRDA_POWER_EN_PIN);  // 红外电源

    gpio_set_pin(IRDA_UART_TXD_PORT, IRDA_UART_TXD_PIN);

    // gpio_enable_pullup(IRDA_UART_RXD_PORT, IRDA_UART_RXD_PIN, 0);
    gpio_enable_pullup(IRDA_HALL_INT_PORT, IRDA_HALL_INT_PIN, 0);
}
...
void irda_deinit(void) {
    sysctrl_enable_peripheral_clk(sysctrl_peripheral_clk_uart1, 0);
    irda_gpio_deinit();
}

void irda_send_byte(uint8_t data) { uart_send_data_poll(IRDA_UART, data); }
